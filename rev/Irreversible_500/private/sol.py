    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        try:
            with subprocess.Popen(
                ...
                valout = int(re.findall("Collected : \d+", valout.decode())[0][12:])
                return valout
        except IndexError:
            pass
...
def find_string(
    ...
    if not reverse:
        for i in range(len(flag_format), length):
            candidate = find_char_at(
                candidate, i, binary_filename, verbose, charset_code
            )
            print(candidate)
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run an instruction counting based timing attack on a given binary."
    )
    parser.add_argument("filename", type=str, help="The file to run taCTF against")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
    ...
    parser.add_argument("-l", "--length", type=int, help="Length of string, if known.")
    parser.add_argument(
        "-c",
        "--charset",
        type=int,
